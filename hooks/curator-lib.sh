#!/usr/bin/env bash
# memory-curator hooks 共享库：定位记忆库、读写状态文件。
...
#   2) 由 cwd 推算 Claude Code 项目记忆：~/.claude/projects/<cwd 把 '/' 换成 '-'>/memory
#   3) 在 ~/.claude 下兜底查找带 MEMORY.md 的 memory 目录
...
resolve_memory_dir() {
  ...
  local found; found="$(find "$HOME/.claude/projects" -maxdepth 3 -name MEMORY.md 2>/dev/null | head -1)"
